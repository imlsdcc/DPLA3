...
var dcc = function () {
	...
	drawBrief = function (briefData, resultDiv) {
	    ...
	    */
	    ...
	    var zotSpan = '<span class="Z3988" title="ctx_ver=Z39.88-2004&rft_val_fmt=info:ofi/fmt:kev:mtx:book&rft_id=http://dpla.grainger.illinois.edu' + briefData.detailurl + '&rft.btitle=' + briefData.title + '&rft.au=' + briefData.creator + '"></span>'
	    $(resultDiv).append(zotSpan);
	    $(resultDiv).append('<abbr class="unapi-id" title="' + escape(briefData.identifier) + '"></abbr>');
    ...
    getFacetValues = function (idList, name, pars) {
        //idList is a comma seperated list of facet ids
        //returns an object of values and the corresponding ids
        var returnObj;
        $.get(serviceUrl + 'Facets/GetFacetValue?ids=' + idList, function (data) {
            for (d in data) {
                returnObj[data[d].facetid] = data[d].text;
            }
            //returnObj = data;
        }).complete(function(){
            dcc.secondarySearches(name, pars, returnObj);
        }).error(function () {
            dcc.secondarySearches(name, pars, "error");
        });
    ...
    return {
        runService: function (url, service, facet) {
            ...
            //                dcc.getFacets(url);
            //            }
            dcc.secondarySearches('hathi', $.deparam.querystring(url));
        ...
        constructWebServiceURL: function (opts, baseURL, collections) {
            ...
            */
            ...
            for (key in params) {
                if (hash[key]) {
                    //var list = '#' + key + hash[key].split(' ').join('_') //.replace(' ', '_')
                    
                    if (jQuery.inArray(key, keyAr) >= 0 && $('#' + key + hash[key].split(' ').join('_')).length == 0) {
                        $('#field-query-ul').append('<li id=\"' + key + hash[key].split(' ').join('_') + '\">' + key.charAt(0).toUpperCase() + key.slice(1) + " : \"" + hash[key] + '\"</li>');
                    }
...
//                }
        ...
        secondarySearches: function (secName, secParams, facetObj) {
            //Search Hathi/Flickr on item string and facets
            //If no keyword or secName doesn't have 'hathi' or 'flickr' it will not search
            //SecName = String - 'hathi' or 'flickr'
            //SecParams = ObjectArray - [keyword string(anywhere),date,place,type]
            var  da = "", pl = "", ty = "";
            if (secParams.idates) { da = secParams.idates }
            if (secParams.iplaces) { pl = secParams.iplaces }
            if (secParams.itypes) { ty = secParams.itypes }
            if (da || pl || ty) {
                getFacetValues(da + ',' + pl + ',' + ty, secName, secParams)
            }
            if (facetObj || (!da && !pl && !ty)) {
                var secURL = '/Service/Search/SecondarySearch?searchtype=' + secName, key = secParams.anywhere, date, place, type, url;
                if (secName == 'hathi') {
                    url = 'http%3A%2F%2Fbabel%2Ehathitrust%2Eorg%2Fcgi%2Fls%3Fq1%3D' + key + '%26lmt%3Dft%26a%3Dsrchls%3Bsz%3D10%3Bpn%3D1'
                    //$('#tabs-hathi').append("<a href='" + url + "'>See the full search results</a>");
                    secURL = secURL + '&url=' + url;
                    for (d in secParams.idates) {

                        //secURL = secURL + '&facet=publishDateRange:"' + secParams.idates[p] + '"';
                    }
                    for (p in secParams.iplaces) {
                        //secURL = secURL + '&facet=countryOfPubStr:"United States ' + secParams.iplaces[p] + '"';
                    }
                    for (t in secParams.itypes) {

                    }
                    //date = secParams.idates;
                    //place = secParams.iplaces;
                    //type = secParams.itypes;

                } else if (secName == 'flickr') {
                    url = 'http%3A%2F%2Fwww%2Eflickr%2Ecom%2Fsearch%2F%3Fz%3Dm%26w%3Dcommons%26q%3D' + key + '%26m%3Dtext'
					//url above no longer working/ 
					//'http%3A%2F%2Fapi.flickr.com%2Fservices%2Frest%2F%3Fmethod%3Dflickr.photos.search%26api_key%3D' + API_Key + '%26text%3D' + key + '%26in_commons%3Dtrue'
					//may work; parsing needs to be checked...
                    //$('#tabs-flickr').append("<a href='"+url+"'>See the full search results</a>");
                    secURL = secURL + '&url=' + url;
                    date = secParams.idates;
                    place = secParams.iplaces;
                    type = secParams.itypes;
                ...
                if (key && (secName == 'hathi' || secName == 'flickr')) {

                    $.get(secURL, function (data) {
                        $('#tabs-' + secName).html("<div class='secondaryUrl'><a href='" + decodeURIComponent(url) + "' target='_blank'>See the full search results for \"" + key + "\" <img id='ExternalLink' name='ExternalLink' alt='External Link' src='../../Images/externallinks.gif'></a></div>" + data);
                    }).error(function () {
                        $('#tabs-' + secName).html('Sorry, there was an error that occured.');
                    }).complete(function () {
                        if (secName == 'hathi') {

                            $('#tabs-hathi .item-match').append('<span class="item-type">Text</span>');
                            $('#tabs-hathi .item-match a').attr('target', '_blank');
                            dcc.drawUnapiTag('hathi');
                            dcc.secondarySearches('flickr', secParams);
                        } else {
                            $('#tabs-flickr .item-match').append('<span class="item-type">Image</span>');
                            $('#tabs-flickr .item-match a').attr('target', '_blank');
                            dcc.drawUnapiTag('flickr');
        ...
        drawUnapiTag: function (div) {
            $('#tabs-' + div + ' .item-match').each(function () {
                $(this).append('<span class="Z3988" title="ctx_ver=Z39.88-2004&amp;rft_val_fmt=info:ofi/fmt:kev:mtx:book&rft_id=' + $(this).find('a').attr('href') + '&rft.btitle=' + $(this).find('a').text() + '&rft.au=' + $(this).find('p').first().text().replace('by ', '') + '&rft.date=' + $(this).find('p').last().text().replace('Published ', '') + '"></span>');

...
//                    img.onerror = function () { alert(this.src); }
        ...
        if($('#detailTab').html() == 'Item'){
            ...
            var zotSpan = '<span class="Z3988" title="ctx_ver=Z39.88-2004&amp;rft_val_fmt=info:ofi/fmt:kev:mtx:book&rft_id=' + window.location + '&rft.btitle=' + json.title + '&rft.au=' + json.creator + '&rft.pub=' + json.publisher + '&rft.date=' + json.dateof + '&rft.genre=Image"></span>'
            $('#details-container').append(zotSpan);
            $('#details-container').append('<abbr class="unapi-id" title="'+ escape(json.identifier) +'"></abbr>');
            //$('#details-container .unapi-id').attr('title', json.identifier)
            //$('.right').append(img);
            //Deal with the lists
            ...
            if (json.itemsInCollection > 0) {
                $('#itemsInCollection-panel').append('<a href="/ResultList/Items/1?collections=' + $('#detailNumber').html() + '">View all ' + json.itemsInCollection + ' items from this collection</a>');
            }
            //COinS implementation
            //var zotSpan = '<span class="Z3988" title="ctx_ver=Z39.88-2004&amp;rft_val_fmt=info:ofi/fmt:kev:mtx:book&rft_id=' + window.location + '&rft.btitle=' + json.title + '&rft.au=' + json.creator + '&rft.pub=' + json.publisher + '&rft.date=' + json.dateof + '&rft.genre=Image"></span>'
            //$('#details-container').append(zotSpan);
            //$('#details-container').append('<abbr class="unapi-id" title="http://unapi.info/news/archives/9"></abbr>');
